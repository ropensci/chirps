...
The **chirps** package provides functionalities for reproducible analysis using the CHIRPS data [@Funk2015] in R[@RCoreTeam]. CHIRPS is daily precipitation a database developed by the Climate Hazards Group[@Funk2015] for high resolution precipitation gridded data. Is a 35+ year quasi-global rainfall data set. Spanning 50°S-50°N (and all longitudes) and ranging from 1981 to near-present, CHIRPS incorporates 0.05 arc-degree resolution satellite imagery, and in-situ station data to create gridded rainfall time series for trend analysis and seasonal drought monitoring[@Funk2015]. Other functionality of **chirps** is the Evaporative Stress Index (ESI) which describes temporal anomalies in evapotranspiration produced weekly at 0.25 arc-degree resolution for the entire globe.
...
With `precip_indices` is possible to track how the precipitation evolves across a time series using precipitation variability indices[@Aguilar2005]. Let's take the indices for periods of 7 days (1-week) and check how these indices changed across the time series.
...
```{r mm, message=TRUE, eval=TRUE, echo=TRUE}
library("tidyverse")
...
Here we see how these indices are changing across the time series. It is possible to see that the maximum precipitation in consecutive 5-days (Rx5day) is higher in the first weeks of the years. Which agree with the beginning of the rainy season in this region. In the other chart we note a increasing in the extent of consecutive dry days (MLDS) across the time series, with also a decrease in the number of consecutive rainy days (MLWS). If used carefully and with scientific rigor, these indices are a good proxy to evaluate the climate variability using precipitation data [@Aguilar2005; @DeSousa2018].  
...
The function `get_esi` may return NA's due to cloudiness in the dataset. One way to turn over this is increase the buffer area around inputted object with the argument 'dist' passed to `sf`.
...
To return an object with the same class (sf), the argument 'as.sf = TRUE' is used. This, however, cannot be passed to `precip_indices`.
