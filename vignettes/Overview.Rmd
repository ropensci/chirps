...
The *Tapajós* National Forest, Brazil is a protected area in the Brazilian Amazon. We take two random points across its area to get the precipitation from Jan-2015 to Dec-2016 using `get_chirps()`.
...
The function `get_esi()` may return `NA`s due to cloudiness in the dataset. One way to deal with this is increase the buffer area around the in-putted object with the argument `dist` passed to `sf()` methods through the `...` functionality in `get_esi()`.
