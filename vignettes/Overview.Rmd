...
With `precip_indices()` is possible to track how the precipitation changes across a time series using precipitation variability indices[@Aguilar2005]. Here, we take the indices for periods of 7 days and compute the indices the time series (from Jan-2013 to Dec-2018).
...
#> 6     1 2013-01-01 -54.9 -2.06 Rx5day  47.7
```
...
One way to deal with this is increase the buffer area around the in-putted object with the argument `dist` passed to `sf()` methods through the `...` functionality in `get_esi()`.
